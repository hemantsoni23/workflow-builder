...
// activepieces/packages/react-ui/src/app/router.tsx
// import { useEffect, useMemo } from 'react';
...
//   return fragment ? (
//     <Navigate to={`/settings/${fragment}`} replace />
//   ) : (
//     <Navigate to="/settings/general" replace />
//   );
// };
...
//       <Route path="/embed/connections" element={<EmbeddedConnectionDialog />} />
...
// A helper component to reroute /settings routes based on hash.
const SettingsRerouter = () => {
  const { hash } = useLocation();
  const fragment = hash.slice(1).toLowerCase();
  return fragment ? (
...
export const ApRouter = () => {
  // If your app supports an "embedded" state, handle it here.
  // For now, we simply render the full set of routes.
  return (
    <Routes>
      {/* Default redirect from base to sign-in */}
